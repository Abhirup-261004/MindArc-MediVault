...
// Find nearby hospitals / medical centers using user's current location
function findNearbyHospitals() {
    ...
    const container = document.getElementById('emergency-results');
    if (container) {
        container.innerHTML = '<p>Looking for nearby hospitals...</p>';
    }
    ...
    navigator.geolocation.getCurrentPosition(
        async function (pos) {
            ...
            // Center map on user for context
            if (map) {
                map.setView([userLat, userLon], 14);
            }

            await queryHospitalsAround(userLat, userLon);
        },
        function (err) {
            console.error(err);
...
// Query Overpass API for hospitals/clinics around given coordinates
async function queryHospitalsAround(lat, lon) {
    clearHospitals();
    ...
    try {
        // 3000m radius around the user
        const overpassQuery = `
            [out:json][timeout:25];
            (
              node["amenity"~"hospital|clinic|doctors"](around:3000,${lat},${lon});
              way["amenity"~"hospital|clinic|doctors"](around:3000,${lat},${lon});
              relation["amenity"~"hospital|clinic|doctors"](around:3000,${lat},${lon});
            );
            out center tags;
        `;
        ...
        if (!response.ok) {
            ...
            container.innerHTML = '<p>Unable to fetch nearby hospitals right now.</p>';
            return;
        }

        const data = await response.json();
        if (!data.elements || data.elements.length === 0) {
            container.innerHTML = '<p>No hospitals found within 3 km radius.</p>';
            return;
        }
        ...
        const hospitals = data.elements.map(el => {
            ...
            const centerLon = el.lon || (el.center && el.center.lon);
        ...
        nearest.forEach(h => {
            ...
            const phoneClean = h.phone && h.phone !== 'Not available'
                ? h.phone.replace(/[^+\d]/g, '')
                : null;
            ...
            card.innerHTML = `
                ...
                <div class="hospital-phone">
                    📞 ${
                        phoneClean
                            ? `<a href="tel:${phoneClean}">${h.phone}</a>`
                            : 'Phone not available'
                    }
                </div>
            `;
            ...
            if (map) {
                const marker = L.marker([h.lat, h.lon], {
                    icon: L.divIcon({
                        html: '<div style="background:#ef4444;color:#fff;width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:18px;border:2px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.4);">H</div>',
                        iconSize: [30, 30],
                        iconAnchor: [15, 15]
                    })
                }).addTo(map).bindPopup(
                    `<strong>${h.name}</strong><br>${h.address}<br>` +
                    (phoneClean ? `📞 ${h.phone}` : 'Phone not available')
                );
        ...
        container.innerHTML = '<p>Something went wrong while searching for hospitals.</p>';
    }
}
